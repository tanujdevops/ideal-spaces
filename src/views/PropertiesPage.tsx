...
const PropertiesPage: React.FC = () => {
  ...
  // Load properties from local frontend data
  useEffect(() => {
  ...
  const filteredProperties = useMemo(() => {
    ...
    switch (sortBy) {
      ...
      case 'beds':
        ...
        break;
      case 'newest':
        // Use id order as a stable local fallback.
          ...
          {error && !loading && (
            <div className="flex items-center justify-center py-24">
              <div className="text-center">
                <span className="material-icons text-4xl text-[#E0B030] mb-4">error_outline</span>
                <p className="font-manrope text-[#374151] mb-4">{error}</p>
                <button
                  onClick={() => window.location.reload()}
                  className="bg-[#E0B030] text-[#221410] font-manrope font-bold px-6 py-2 rounded-lg hover:bg-[#C8911F] transition-all"
                >
                  Retry
                </button>
              </div>
